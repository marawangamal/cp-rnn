...
def bpc_plot(args):
    ...
    for filename in os.listdir(args['visualization']['experiments']):
        ...
        if not satisfies_conditions(exp_cfg, args, catchall='any') or dct['epoch'] < min_epochs:
            ...
            continue
        elif dct['epoch'] < min_epochs:
            print("  Skipping: {} | Too Few Epochs: {}".format(filename, dct['epoch']))
            continue
        ...
        group_name = ", ".join([str(get_leaf_value(exp_cfg, attr_name)) for
    ...
    for group_name, (params, bpc) in groups.items():
        plt.scatter(params, bpc, label=group_name, marker=next(marker))
